...
def read_nutritional_data(file_path= f'{APP_DIR}/food_composition_table.csv',view=False):
...
def nutrional_data_view(ds):
    ...
    for col1,col2 in zip(ds.columns,ds.columns[1:]):
        if col1.replace('_min','') == col2.replace('_max',''):
            dq[col1.replace('_min','')] = ds[[col1,col2]].round(3).apply(lambda x: str(x[0])  +'-' + str(x[1]) if x[0]!=x[1] else x[0],axis=1)
    return dq
