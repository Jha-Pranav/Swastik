...
class Base(models.Model):
    ...
    def add_columns_from_dataframe(cls, dataframe):
        for column in dataframe.columns:
            field_type, default_value = cls.get_field_type_and_default_value(dataframe[column].dtype)
            if not hasattr(cls, column):
                if field_type == models.CharField:
                    cls.add_to_class(column, field_type(default=default_value,max_length=2000))
                else:
...
from apps.algo_apps.load_pandas_df.food_composition_table import read_nutritional_data, read_columns_mapping,read_indian_food_database
